...
```terminal
PYENV_VERSION=3.8.12 pyenv exec pip install --quiet --disable-pip-version-check --upgrade pip-tools
```

Then to upgrade a dependency to the latest version:

```terminal
PYENV_VERSION=3.8.12 pyenv exec pip-compile --allow-unsafe --quiet --generate-hashes --upgrade-package <FOO> requirements/<ENV>.in
make requirements
```
...
```terminal
PYENV_VERSION=3.8.12 pyenv exec pip-compile --allow-unsafe --quiet --generate-hashes --upgrade-package <FOO>==<X.Y.Z> requirements/<ENV>.in
make requirements
```
...
```terminal
PYENV_VERSION=3.8.12 pyenv exec pip-compile --allow-unsafe --quiet --generate-hashes --upgrade <FOO> requirements/<ENV>.in
make requirements
```
