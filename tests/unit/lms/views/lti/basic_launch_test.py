...
class TestBasicLaunchViews:
    def test___init___(self, pyramid_request, grouping_service, course_service):
        BasicLaunchViews(pyramid_request)
    ...
    def test__init___doesnt_update_grading_info_for_instructors(
        self, pyramid_request, grading_info_service
    ):
        BasicLaunchViews(pyramid_request)
    ...
    def test_unconfigured_launch(self, svc, pyramid_request, js_config):
        ...
        pyramid_request.lti_params.serialize.assert_called_once_with(
            authorization=js_config.auth_token
        )
    ...
    def test_unconfigured_launch_not_authorized(
        self, pyramid_request, has_permission, js_config
    ):
        has_permission.return_value = False

        response = BasicLaunchViews(pyramid_request).unconfigured_launch()
        ...
        assert response == {"js_config": js_config.asdict.return_value}

    def test_reconfigure_assignment_config(
        self, svc, js_config, pyramid_request, assignment_service
    ):
        pyramid_request.lti_params = mock.create_autospec(
            LTIParams, spec_set=True, instance=True
        )
        ...
        assert response == {
            ...
            "filePicker": js_config.enable_file_picker_mode.return_value["filePicker"],
        }
    ...
    def test__show_document(
        ...
        js_config.enable_lti_launch_mode.assert_called_once_with(
            course_service.get_from_launch.return_value, assignment
        )
        js_config.set_focused_user.assert_not_called()
        js_config.add_document_url.assert_called_once_with(sentinel.document_url)

        assert result == {}

    @pytest.mark.usefixtures("with_canvas")
    def test__show_document_focuses_on_users(self, svc, pyramid_request, js_config):
        ...
        js_config.set_focused_user.assert_called_once_with(sentinel.focused_user)

    def test__show_document_focuses_on_users_only_for_canvas(
        self, svc, pyramid_request, js_config
    ):
        pyramid_request.params["focused_user"] = sentinel.focused_user
    ...
    def test__show_document_enables_instructor_toolbar_for_instructors(
        self, svc, js_config, assignment_fixture_name, request, enable_grading
    ):
    ...
    def test__show_document_does_not_enable_instructor_toolbar_in_canvas(
        self, svc, application_instance, js_config
    ):
    ...
    def test__show_document_enables_speedgrader_settings(self, svc, js_config):
        svc._show_document(sentinel.document_url)  # pylint: disable=protected-access
    ...
    def test__show_document_no_speedgrader_without_canvas(self, svc, js_config):
        svc._show_document(sentinel.document_url)  # pylint: disable=protected-access
    ...
    def test__show_document_no_speedgrader_with_instructor(self, svc, js_config):
        svc._show_document(sentinel.document_url)  # pylint: disable=protected-access
    ...
    def test__show_document_no_speedgrader_without_grading_id(self, svc, js_config):
        svc._show_document(sentinel.document_url)  # pylint: disable=protected-access
    ...
    def svc(self, pyramid_request):
        return BasicLaunchViews(pyramid_request)

    @pytest.fixture
    def _show_document(self, svc):
        with patch.object(svc, "_show_document") as _show_document:
    ...
    def js_config(self, patch):
        return patch(
            "lms.views.lti.basic_launch.JSConfig", spec_set=True, autospec=JSConfig
        ).return_value
