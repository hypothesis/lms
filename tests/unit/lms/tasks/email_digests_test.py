        ...
        assert send_instructor_email_digest.apply_async.call_args_list == [
            call(
                (),
                {
                    ...
                    "created_after": "2023-03-08T05:00:00+00:00",
                },
            )
...
class TestSendInstructorEmailDigests:
    def test_it(self, created_after, created_before, digest_service, h_userid):
        send_instructor_email_digest(
            h_userid=h_userid,
        ...
        digest_service.send_instructor_email_digest.assert_called_once_with(
            h_userid="test_h_userid",
            created_after=created_after,
            created_before=created_before,
            override_to_email=sentinel.override_to_email,
        )

    def test_it_crashes_if_created_after_is_invalid(self, h_userid, created_before):
    ...
    def test_it_crashes_if_created_before_is_invalid(self, h_userid, created_after):
        with pytest.raises(ValueError, match="^Invalid isoformat string"):
            send_instructor_email_digest(
                h_userid=h_userid,
                created_after=created_after.isoformat(),
    ...
    @pytest.fixture
    def created_after(self, created_before):
        """Return the created_after arg that will be passed to send_instructor_email_digest()."""
