...
class TestUpsertGradingInfo:
    def test_it_creates_new_record_if_no_matching_exists(
        self, svc, application_instance, pyramid_request, params, db_session
    ):
        result = svc.upsert_grading_info(**params)

        assert result == db_session.query(GradingInfo).one_or_none()
        ...
        assert result.lis_result_sourcedid == params["lis_result_sourcedid"]
        assert result.lis_outcome_service_url == params["lis_outcome_service_url"]
        assert result.user_id == pyramid_request.lti_user.user_id
        assert result.application_instance_id == application_instance.id
        assert result.context_id == params["context_id"]
        assert result.resource_link_id == params["resource_link_id"]

    def test_it_updates_existing_record_if_matching_exists(
        self, svc, pyramid_request, lti_user, application_instance, params
    ):
        ...
        svc.upsert_grading_info(**params)
    ...
    def test_it_does_nothing_with_required_parameter_missing(self, svc, params, param):
        del params[param]

        assert not svc.upsert_grading_info(**params)
