...
def via_url(_context, request):
    ...
    # TODO we need to use via.secret.query or similar
    url = DOCUMENT_URL_REGEX.search(document_url)["url"] + "&token="
    return {
        "via_url": helpers.via_url(request, url, content_type="pdf", headers=headers)
    }
