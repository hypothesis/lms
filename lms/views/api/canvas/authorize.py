...
class CanvasAPIAuthorizeViews:
    ...
    def authorize(self):
        ...
        if self._sections_enabled(ai_getter):
    ...
    @staticmethod
    def _sections_enabled(ai_getter):
        if not ai_getter.canvas_sections_supported():
            return False

        return ai_getter.get().settings("canvas", "sections_enabled")
