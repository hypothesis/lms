...
CREATE TYPE report.academic_timescale AS ENUM ('week', 'month', 'semester', 'academic_year', 'all_time');
...
CREATE MATERIALIZED VIEW report.organization_activity AS
  (WITH -- In order to get all the combinations of different types of data we
 ...
 weeks AS
     (SELECT timestamp_week
      FROM report.events
      UNION SELECT created_week AS timestamp_week
      FROM report.group_activity),
 timescales AS
     (SELECT column1 AS timescale
      FROM (
            VALUES ('week'), ('month'), ('semester'), ('academic_year'), ('all_time')) AS data),
 roles AS
     (SELECT column1::report.roles AS role
      FROM (
            VALUES ('user'), ('teacher')) AS data), periods AS
     (SELECT timestamp_week,
     ...
     (SELECT *
      FROM periods
      CROSS JOIN roles), -- Then we join onto this facets table and use it to group our data in
 -- all of the different combos.
 user_annotation_counts AS
     (SELECT period,
             timescale,
             group_map.organization_id,
             SUM(annotation_count) AS annotation_count -- We don't have annotation counts by teacher, so this is user only
 -- so we'll use "periods" instead of the full "facets"

      FROM periods
      JOIN report.group_activity ON group_activity.created_week = periods.timestamp_week
      JOIN report.group_map ON group_activity.group_id = group_map.group_id
      GROUP BY period,
               timescale,
               group_map.organization_id), -- We calculate this differently from the group above because it's
 ...
 -- so we'll use "periods" instead of the full "facets"
      ...
      FROM periods
      JOIN report.user_activity ON user_activity.created_week = periods.timestamp_week
      JOIN report.organization_roles ON user_activity.user_id = organization_roles.user_id
      AND organization_roles.role = 'teacher'
      GROUP BY period,
               timescale,
               organization_id),
 active AS
     (SELECT facets.period,
             facets.timescale,
             facets.role,
             events.organization_id,
             COUNT(DISTINCT(events.user_id)) AS active
      FROM facets
      JOIN report.events ON events.timestamp_week = facets.timestamp_week
      JOIN report.organization_roles ON organization_roles.organization_id = events.organization_id
      AND organization_roles.user_id = events.user_id
      AND organization_roles.role = facets.role
      WHERE event_type = 'configured_launch'
      GROUP BY facets.period,
               facets.timescale,
               facets.role,
               events.organization_id),
 billable AS
     (SELECT facets.period,
      ...
      FROM facets
      JOIN report.group_activity ON group_activity.created_week = facets.timestamp_week
      JOIN report.group_map ON group_activity.group_id = group_map.group_id
      JOIN report.user_groups ON user_groups.group_id = group_activity.group_id
      JOIN report.organization_roles ON organization_roles.organization_id = group_map.organization_id
      AND organization_roles.user_id = user_groups.user_id
      AND organization_roles.role = facets.role
      GROUP BY facets.period,
               facets.timescale,
               facets.role,
               group_map.organization_id), -- We need to combine the data. A join seems the most natural way of
 ...
 unioned_metrics AS
     (SELECT period,
      ...
      UNION ALL SELECT period,
                       ...
                       0,
                       billable
      FROM billable) -- We can then finally MAX over the columns to get a single row with each
