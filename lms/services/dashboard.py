...
class DashboardService:
    ...
    def get_request_segment(self, request, authority_provided_id: str) -> LMSSegment:
        """Get and authorize an segment for the given request."""
    ...
    def get_segments_roster(
        ...
        rosters_last_updated = self._get_segment_rosters_last_updated(segments)
        # Only use roster data if we have it for all segments, othwrwise it becomes tricky to build a coeherent roster
        if rosters_enabled and rosters_last_updated:
            # Use the oldest last updated date as the last updated date for the combined roster
            roster_last_updated = min(rosters_last_updated)
            # If rostering is enabled and we do have the data, use it
    ...
    def _get_segment_rosters_last_updated(
        ...
        """Return a list of dates with te last updated date for the rosters of the segments.

        It will return None if we don't have a roster for any of the segments.
        """
        ...
        if all(rosters_last_updated):
            return rosters_last_updated  # type: ignore
