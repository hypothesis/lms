...
def upgrade():
    ...
    Querying for just:
        ...
        SELECT COUNT(id) FROM application_instances WHERE developer_key IS NOT NULL

    returns 957 AIs at the time of writing

    while trying to get the 'custom_canvas_course_id' part:
    ...
    but removing the last condition, the one with custom_canvas_course_id gives us the same number.

    This means that all canvas instances that have a developer key and have created courses do send custom_canvas_course_id.
    """
    conn = op.get_bind()
    result = conn.execute(
        """
