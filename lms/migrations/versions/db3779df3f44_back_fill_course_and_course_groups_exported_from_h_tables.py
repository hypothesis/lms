    ...
    for group in groups:
        ...
        if created.date() > datetime.date(2020, 5, 18):
            for app_instance in app_instances:
                ...
                if inserted:
                    rows_inserted += 1
                    rows_inserted_with_sections_enabled += 1
        ...
        rows_inserted += 1
        rows_inserted_with_sections_disabled += 1
        maybe_commit(session, rows_inserted)

    session.commit()

    log.info(
        f"Upserted {rows_inserted_with_sections_enabled} rows with a non-null consumer"
        " key and sections enabled"
    )
    log.info(
        f"Upserted {rows_inserted_with_sections_disabled} rows with a NULL consumer key"
        " and sections DISABLED"
    )
    log.info(f"Upserted {rows_inserted} rows in total")
...
def course_groups(session):
    """Return just the course groups from groups() (filter out section groups)."""
    return [group for group in groups() if is_course_group(session, group)]
...
# FIXME: Update this with the latest groups.
def groups():
